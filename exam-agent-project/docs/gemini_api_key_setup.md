...
python .\src\evaluation.py --blueprint nonexistent_path --provider gemini --quality final --strict-provider --simulate-trials 1
```

The built-in `final` profile uses Gemini Flash for planner/writer/answering and
Flash-Lite for repeated judge calls. Use `--model-preset pro` only when you
intentionally want the higher-cost Pro preset. The provider retries after
429/resource-exhausted errors and can fall back to configured lower-cost models
such as `gemini-2.5-flash-lite` before failing.
